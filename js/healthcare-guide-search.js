document.addEventListener("DOMContentLoaded", () => {
  const searchInput = document.getElementById("healthcareGuideSearchInput");
  const resultsContainer = document.getElementById("healthcareGuideSearchResults");
  ...
  const guides = guideColumns.map((column) => {
    ...
    return {
      id: column.dataset.guideId ?? title.toLowerCase(),
      column,
      title,
      description,
      url,
  ...
  const renderResults = (matches) => {
    if (!matches.length) {
      showMessage("No guides match that term. Try a different keyword.");
  ...
  const handleSearch = (value) => {
    ...
    if (term.length < MIN_TERM_LENGTH) {
      ...
      showMessage("Start typing to surface guides or jump directly to an article.");
      return;
    }
